...
{
    public class TlsMqContainer : IAsyncLifetime
    {
        ...
        private readonly ContainerImage _image;

        public TlsMqContainer(ContainerImage image)
        {
            _image = image;
        }

        public TlsMqContainer BuildWithTls(string mqStartupScriptPath, bool exposeWebConsole)
        {
            var builder = new ContainerBuilder()
                .WithImage(_image.Name)
        ...
        public ConnectionOptions GetTlsConnectionOptions() => new ConnectionOptions
        {
            QueueManagerName = MqManagerName,
            ConnectionName = $"localhost({_container.GetMappedPublicPort(MqPort)})",
            UserId = MqAppUser,
            Password = Password,
            UseTls = true,
            SslCipherSpec = "ANY_TLS12_OR_HIGHER", // Support elliptic curve ciphers
            SslKeyRepository = GetClientKeyRepository(),
            SslKeyRepositoryPassword = "password",
            SslCertLabel = "ibmwebspheremq",
            SslFipsRequired = false,
            SslPeerNameRequired = false
        };

        private string GetClientKeyRepository()
        {
            // Return path to client certificates for TLS connection
            var clientCertsPath = Path.Combine(Directory.GetCurrentDirectory(), "..", "..", "..", "..", "certs", "client", "client");
            return Path.GetFullPath(clientCertsPath);
        }

        public int GetMappedPort() => _container.GetMappedPublicPort(MqPort);
    }
}
